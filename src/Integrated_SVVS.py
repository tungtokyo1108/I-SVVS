...
class Integrated_SVVS():
    ...
    def _initialize_parameters(self, X, Y, random_state):
        ...
        # Intergated clustering
        self._estimate_weights_gl()
    ...
    def _e_step_mt(self, X):
        
        log_prob_norm, log_resp = self._estimate_log_prob_resp_mt(X)
        prob_selected = self._estimate_prob_selection_mt(X)
        return np.mean(log_prob_norm), log_resp, prob_selected 
    
    def _m_step_mt(self, X, log_resp_mt): 
        
        n_samples, n_features = X.shape
        
        self.resp_mt = np.exp(log_resp_mt)
        resp = np.dot(self.resp_gl, self.resp_mt)
        nk = np.dot(resp.T, self.selected_mt) + 10 * np.finfo(resp.dtype).eps
        xk = np.dot(resp.T, X * self.selected_mt) / nk 
        sk = {"full": _estimate_gaussian_covariances_full,
              "tied": _estimate_gaussian_covariances_tied,
              "diag": _estimate_gaussian_covariances_diag,
              }[self.covariance_type](self.resp_gl, self.resp_mt, X, self.selected_mt, nk, xk, self.reg_covar)
    ...
    def _estimate_log_prob_mc(self, Y):
        ...
        sum_2_2 = self.resp_gl * digamma(sum_2_2)
        sum_2_2 = np.dot(np.dot(sum_2_2.T, self.selected_mc), (means_mc * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis = 1))[:, np.newaxis] - log_means_mc)).T)
        ...
        for k in range(self.n_components):
            y = Y + means_mc[k]
            y = self.selected_mc * gammaln(y)
            sum_3_1[:, k] = np.sum(y, axis = 1)
        sum_3_1 = np.dot(self.resp_gl.T, sum_3_1)
        sum_3_2 = np.dot(np.dot(self.resp_gl.T, self.selected_mc), gammaln(means_mc).T)
        ...
        for k in range(self.n_components):
            y = Y + means_mc[k]
            y = self.selected_mc * digamma(y) * means_mc[k] * (digamma(self.gamma_vi)[k] - digamma(self.gamma_vi.sum(axis = 1))[k] - log_means_mc[k])
            sum_4_1[:, k] = np.sum(y, axis = 1)
        sum_4_1 = np.dot(self.resp_gl.T, sum_4_1)
        sum_4_2 = np.dot(np.dot(self.resp_gl.T, self.selected_mc), (means_mc * digamma(means_mc) * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis = 1))[:, np.newaxis] - log_means_mc)).T)
        part_4 = sum_4_1 - sum_4_2
        
        
        return part_1 + part_2 + part_3 + part_4
    
    def _estimate_weighted_log_prob_mc(self, Y):
        
        return self._estimate_log_prob_mc(Y) + self._estimate_log_weights_mc()
    
    def _estimate_log_prob_resp_mc(self, Y):
        
        weight_log_prob = self._estimate_weighted_log_prob_mc(Y)
        log_prob_norm = logsumexp(weight_log_prob, axis = 1)
        with np.errstate(under = 'ignore'):
            log_resp = weight_log_prob - log_prob_norm[:, np.newaxis]
            
        return log_prob_norm, log_resp
    
    def _estimate_log_prob_selected_mc(self, Y):
        ...
        for k in range(self.n_components):
            y = Y + means_mc[k]
            sum_1_1[:, k] = np.sum(y, axis = 1)
        part_1 = (join_clus_mc * gammaln(means_mc.sum(axis=1)) - join_clus_mc * gammaln(sum_1_1)).sum(axis = 1)
        
        
        sum_2_1 = means_mc * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis = 1))[:, np.newaxis] - log_means_mc) * digamma(means_mc.sum(axis = 1))[:, np.newaxis]
        sum_2_1 = np.dot(join_clus_mc, sum_2_1)
        sum_2_2 = np.empty((n_samples, self.n_components))
        for k in range(self.n_components):
            y = Y + means_mc[k]
            sum_2_2[:, k] = np.sum(y, axis = 1)
        sum_2_2 = np.dot((join_clus_mc * digamma(sum_2_2)), (means_mc * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis = 1))[:, np.newaxis] - log_means_mc)))
        part_2 = sum_2_1 - sum_2_2 
        ...
        for k in range(self.n_components):
            y = Y + means_mc[k]
            y = join_clus_mc[:, k][:, np.newaxis] * gammaln(y)
            sum_3_1[k] = y
        sum_3_1 = np.sum(sum_3_1, axis = 0)
        sum_3_2 = np.dot(join_clus_mc, gammaln(means_mc))
        part_3 = sum_3_1 - sum_3_2
        ...
        for k in range(self.n_components):
            y = Y + means_mc[k]
            y = join_clus_mc[:,k][:, np.newaxis] * digamma(y) * means_mc[k] * (digamma(self.gamma_vi)[k] - digamma(self.gamma_vi.sum(axis = 1))[k] - log_means_mc[k])
            sum_4_1[k] = y
        sum_4_1 = np.sum(sum_4_1, axis = 0)
        sum_4_2 = np.dot(join_clus_mc, (means_mc * digamma(means_mc) * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis = 1))[:, np.newaxis] - log_means_mc)))
        part_4 = sum_4_1 - sum_4_2
        
        
        estimate_log_select = part_2 + part_3 + part_4 
        estimate_log_select = estimate_log_select + (digamma(self.xi1_mc) - digamma(self.xi1_mc + self.xi2_mc))
        return estimate_log_select
    
    def _estimate_log_prob_rejected_mc(self, Y):
        
        n_samples, n_features = Y.shape
        
        means_mc = self.iota_vi / self.iota_vi.sum()
        log_means_mc = np.log(means_mc)
        
        
        part_1 = gammaln(means_mc.sum()) - gammaln((Y + means_mc).sum(axis = 1))
        
        
        sum_2_1 = means_mc * (digamma(self.iota_vi) - digamma(self.iota_vi.sum()) - log_means_mc) * digamma(means_mc.sum())
        sum_2_2 = means_mc * (digamma(self.iota_vi) - digamma(self.iota_vi.sum()) - log_means_mc) * digamma((Y + means_mc).sum(axis = 1))[:, np.newaxis]
        part_2 = sum_2_1 - sum_2_2
        
        
        part_3 = gammaln((Y + means_mc)) - gammaln(means_mc)
        
        
        sum_4_1 = means_mc * (digamma(self.iota_vi) - digamma(self.iota_vi.sum()) - log_means_mc) * digamma((Y + means_mc))
        sum_4_2 = means_mc * (digamma(self.iota_vi) - digamma(self.iota_vi.sum()) - log_means_mc) * digamma((means_mc))
        part_4 = sum_4_1 - sum_4_2
        
        
        estimate_log_reject = part_2 + part_3 + part_4 
        estimate_log_reject = estimate_log_reject + (digamma(self.xi2_mc) - digamma(self.xi1_mc + self.xi2_mc))
        return estimate_log_reject
    
    def _estimate_prob_selection_mc(self, Y): 
        
        selection = self._estimate_log_prob_selected_mc(Y)
        rejection = self._estimate_log_prob_rejected_mc(Y)
        ...
        self.selected_mc = (select_exp + 1e-6) / (select_exp + reject_exp + 1e-6)
        
        return self.selected_mc
    
    def _e_step_mc(self, Y):
        
        log_prob_norm, log_resp = self._estimate_log_prob_resp_mc(Y)
        prob_selected = self._estimate_prob_selection_mc(Y)
        return np.mean(log_prob_norm), log_resp, prob_selected
    
    def _m_step_mc(self, Y, log_resp_mc):
        
        n_sample, n_features = Y.shape
        
        self.resp_mc = np.exp(log_resp_mc)
        
        self._estimate_weights_mc()
        self._estimate_selections_mc()
        self._estimate_alpha_mc(Y)
        self._estimate_beta_mc(Y)
    
    
    #------------------------------------------------------------------------------
    # Global clustering 
    # Integreate microbiome + metabolome 
    # Input data: 
    #    X: metabolomics
    #    Y: microbiomics
    #------------------------------------------------------------------------------
 
    def _estimate_weights_gl(self):
        
        nk = self.resp_gl.sum(axis=0) + 10 * np.finfo(self.resp_gl.dtype).eps
        
        self.weight_concentration_gl = (
            1. + nk,
            (self.weight_concentration_prior_gl + 
             np.hstack((np.cumsum(nk[::-1])[-2::-1], 0))))
    
    def _estimate_log_weights_gl(self):
        
        digamma_sum = digamma(self.weight_concentration_gl[0] + 
                              self.weight_concentration_gl[1])
        digamma_a = digamma(self.weight_concentration_gl[0])
        digamma_b = digamma(self.weight_concentration_gl[1])
        
        return (digamma_a - digamma_sum + 
                np.hstack((0, np.cumsum(digamma_b - digamma_sum)[:-1])))
    
    def _estimate_log_prob_gl(self, X, Y):
        ...
        Metabolomics
        """
        n_samples, n_features_mt = X.shape
        n_components_mt, _ = self.means_mt.shape
        
        log_det = _compute_log_det_cholesky(self.precisions_cholesky_mt, self.covariance_type, n_features_mt)
        
        if self.covariance_type == 'full':
            log_prob_mt = np.empty((n_samples, n_components_mt))
            for k, (mu, prec_chol) in enumerate(zip(self.means_mt, self.precisions_cholesky_mt)):
                y = np.dot(X, prec_chol) - np.dot(mu, prec_chol)
                log_prob_mt[:, k] = np.sum(np.square(y) * self.selected_mt * self.degrees_of_freedom_mt[k], axis=1)
            
        elif self.covariance_type == 'diag':
            precisions = self.degrees_of_freedom_mt * (self.precisions_cholesky_mt ** 2)
            log_prob_mt = (np.dot(self.selected_mt, (self.means_mt ** 2 * precisions).T) - 
                           2. * np.dot(self.selected_mt * X, (self.means_mt * precisions).T) + 
                           np.dot(self.selected_mt * (X ** 2), precisions.T))
            
        log_gauss_mt = (-.5 * (n_features_mt * np.log(2 * np.pi) + log_prob_mt)) + .5 * np.dot(self.selected_mt, log_det.T)
        
        log_lambda_mt = n_features_mt * np.log(2.) + digamma(.5 * (self.degrees_of_freedom_mt - np.arange(0, n_features_mt)))
        log_lambda_mt = .5 * (log_lambda_mt - n_features_mt / self.mean_precision_mt)
        
        log_cluster_mt = log_gauss_mt + np.dot(self.selected_mt, log_lambda_mt.T)
        log_cluster_mt = np.dot(log_cluster_mt, self.resp_mt.T)
        
        
        """
        Microbiomics
        """
        n_samples, n_features_mc = Y.shape 
        means_mc = self.gamma_vi / self.gamma_vi.sum(axis=1)[:, np.newaxis]
        log_means_mc = np.log(means_mc)
        ...
        sum_2_1 = means_mc * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis=1))[:, np.newaxis] - log_means_mc) * digamma(means_mc.sum(axis=1))[:, np.newaxis]
        sum_2_1 = np.dot(self.selected_mc, sum_2_1.T)
        sum_2_2 = np.empty((n_samples, self.n_components))
        for k in range(self.n_components):
            y = Y + means_mc[k]
            sum_2_2[:, k] = np.sum(y, axis=1)
        sum_2_2 = digamma(sum_2_2) * np.dot(self.selected_mc, (means_mc * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis=1))[:, np.newaxis] - log_means_mc)).T)
        part_2 = sum_2_1 - sum_2_2
        ...
        for k in range(self.n_components):
            ...
            y = self.selected_mc * gammaln(y)
            sum_3_1[:, k] = np.sum(y, axis=1)
        sum_3_2 = np.dot(self.selected_mc, gammaln(means_mc).T)
        part_3 = sum_3_1 - sum_3_2
        ...
        for k in range(self.n_components):
            ...
            y = self.selected_mc * digamma(y) * means_mc[k] * (digamma(self.gamma_vi)[k] - digamma(self.gamma_vi.sum(axis=1))[k] - log_means_mc[k])
            sum_4_1[:, k] = np.sum(y, axis=1)
        sum_4_2 = np.dot(self.selected_mc, (means_mc * digamma(means_mc) * (digamma(self.gamma_vi) - digamma(self.gamma_vi.sum(axis=1))[:, np.newaxis] - log_means_mc)).T)
        part_4 = sum_4_1 - sum_4_2 
        
        log_cluster_mc = part_1 + part_2 + part_3 + part_4 
        log_cluster_mc = np.dot(log_cluster_mc, self.resp_mc.T)
        
        """
        Integrated clustering
        """
        log_cluster_gl = log_cluster_mt + log_cluster_mc
        return log_cluster_gl
    
    def _estimate_weighted_log_prob_gl(self, X, Y): 
        
        """
        X: metabolomics
        Y: microbiomics
        """
        return self._estimate_log_prob_gl(X, Y) + self._estimate_log_weights_gl()
    
    def _estimate_log_prob_resp_gl(self, X, Y):
        
        weighted_log_prob = self._estimate_weighted_log_prob_gl(X, Y)
        log_prob_norm = logsumexp(weighted_log_prob, axis = 1)
        with np.errstate(under = 'ignore'):
            log_resp = weighted_log_prob - log_prob_norm[:, np.newaxis]
    ...
    def _e_step_gl(self, X, Y):
        
        log_prob_norm, log_resp = self._estimate_log_prob_resp_gl(X, Y)
        return np.mean(log_prob_norm), log_resp
    
    
    #------------------------------------------------------------------------------
    # Main loop estimation 
    # Input data: 
    #    X: metabolomics
    #    Y: microbiomics
    #------------------------------------------------------------------------------
    ...
    def fit_predict(self, X, Y): 
        ...
        random_state = check_random_state(self.random_state)
        n_samples, n_feature_mt = X.shape 
        n_samples, n_feature_mc = Y.shape 
        ...
        for init in range(n_init):
            ...
            for n_iter in range(1, self.max_iter + 1):
                ...
                log_prob_norm_gl, log_resp_gl = self._e_step_gl(X, Y)
                self.resp_gl = np.exp(log_resp_gl)
                clus_update_gl[n_iter] = self.resp_gl
                
                # Update metabolomics clustering
                log_prob_norm_mt, log_resp_mt, prob_selected_mt = self._e_step_mt(X)
                self._m_step_mt(X, log_resp_mt)
                clus_update_mt[n_iter] = self.resp_mt
                sel_update_mt[n_iter] = self.selected_mt
                
                # Update microbiome clustering
                log_prob_norm_mc, log_resp_mc, prob_selected_mc = self._e_step_mc(Y)
                self._m_step_mc(Y, log_resp_mc)
                clus_update_mc[n_iter] = self.resp_mc
                sel_update_mc[n_iter] = self.selected_mc
                
        _, log_resp_gl = self._e_step_gl(X, Y)
        _, log_resp_mt, prob_selected_mt = self._e_step_mt(X)
        _, log_resp_mc, prob_selected_mc = self._e_step_mc(Y)
        
        return log_resp_gl, clus_update_gl, log_resp_mt, prob_selected_mt, log_resp_mc, prob_selected_mc
